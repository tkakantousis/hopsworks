 ...
 */
/**
 * Controller for the process of creating a new feature group using the modal
 */
angular.module('hopsWorksApp')
    .controller('createFeaturegroupCtrl', ['$uibModalInstance', 'FeaturestoreService',
        'growl', 'projectId', 'featurestore', 'ModalService', 'jobs', 'UtilsService', '$scope', 'featuregroups',
        function ($uibModalInstance, FeaturestoreService, growl, projectId, featurestore,
                  ModalService, jobs, UtilsService, $scope, featuregroups) {
            /**
             * Initialize state
             */
            var self = this;
            ...
            for (i = 0; i < self.featuregroups.length; i++) {
                self.featuregroupNames.push(self.featuregroups[i].featuregroupName)
            }
            ...
            self.createFeaturegroup = function () {
                ...
                for (i = 0; i < self.featuregroupNames.length; i++) {
                    if (self.featuregroupName === self.featuregroupNames[i]) {
                        self.featuregroupNameNotUnique = -1;
                        self.wrong_values = -1;
                    }
                }
