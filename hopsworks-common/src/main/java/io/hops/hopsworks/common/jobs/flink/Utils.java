...
public final class Utils {
   ...
   * @param relativeTargetPath relative target path of the file (will be
   * prefixed be the full home directory we set up)
   *
  ...
  /**
   * Deletes the YARN application files, e.g., Flink binaries, libraries, etc.,
  ...
  /**
   * Copied method from org.apache.hadoop.yarn.util.Apps. It was broken by
   * YARN-1824 (2.4.0) and fixed for 2.4.1 by
   * https://issues.apache.org/jira/browse/YARN-1931
   */
  ...
  /**
   * Method to extract environment variables from the flinkConfiguration based
   * on the given prefix String.
   *
  ...
  /**
   * Creates the launch context, which describes how to bring up a TaskExecutor
   * / TaskManager process in an allocated YARN container.
   *
   * <p>
   * This code is extremely YARN specific and registers all the resources that
   * the TaskExecutor needs (such as JAR file, config file, ...) and all
   * environment variables in a YARN container launch context. The launch
   * context then ensures that those resources will be copied into the
   ...
   * @param taskManagerConfig The configuration for the TaskExecutors.
   * @param workingDirectory The current application master container's working
   * directory.
   ...
   *
   * @throws Exception Thrown if the launch context could not be created, for
   * example if the resources could not be copied.
   */
  ...
  /**
   * Validates a condition, throwing a RuntimeException if the condition is
   * violated.
   *
   * @param condition The condition.
   * @param message The message for the runtime exception, with format variables
   * as defined by {@link String#format(String, Object...)}.
